...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/80">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-950 border-b border-slate-800 px-4 pt-2 pb-4 space-y-1 animate-fade-in">
          <Link 
            to="/" 
          ...
          <div className="pt-2">
            <Link 
              to="/movies" 
              onClick={() => setMobileMenuOpen(false)}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 text-white font-medium text-sm shadow-md shadow-rose-600/30"
            >
              <Sparkles className="w-4 h-4" />
              <span>Explore Now</span>
            </Link>
